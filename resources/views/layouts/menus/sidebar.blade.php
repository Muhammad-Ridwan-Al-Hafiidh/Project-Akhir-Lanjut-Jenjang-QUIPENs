<!-- Sidebar -->
<ul class="navbar-nav sidebar sidebar-light accordion" id="accordionSidebar">

    <!-- Sidebar - Brand -->
    <a class="sidebar-brand text-lg d-flex align-items-center justify-content-center" href="{{ route('home') }}">
    <div class="text-primary sidebar-brand-text mx-2 fs-3">QuiPenz</div>
</a>

    <!-- Divider -->
    <hr class="sidebar-divider my-0">

    <!-- Nav Item - Dashboard -->
    <li class="nav-item">
        <a class="nav-link" href="{{ route('dashboard') }}">
            <i class="fas fa-fw fa-tachometer-alt"></i>
            <span>{{ __("Dashboard") }}</span></a>
    </li>

    @role('student')
    <!-- Divider -->
    <hr class="sidebar-divider">
    <!-- Heading -->
    <div class="sidebar-heading">
        {{ __('Course Management') }}
    </div>
    <li class="nav-item">
        <a class="nav-link" href="{{ route('myCourse') }}">
            <i class="fas fa-fw fa-book-reader"></i>
            <span>{{ __("My Course") }}</span></a>
    </li>
    <!-- Divider -->
    <hr class="sidebar-divider">
    @endrole

    @can('menu.education')
    <!-- Heading -->
    <div class="sidebar-heading">
        {{ __('Education') }}
    </div>

    @can('mentor.list')
    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#MentorsCollapse" aria-expanded="true" aria-controls="ACLCollapse">
            <i class="fas fa-fw fa-clipboard"></i>
            <span>Mahasiswa</span>
        </a>
        <div id="MentorsCollapse" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item" href="{{ route('myLearners') }}">{{ __('My Learners') }}</a>
            </div>
        </div>
    </li>
    @endcan

    @can('course.index')
    @endcan

    @endcan

    @role('Super-Admin')

    <div class="sidebar-heading">
        {{ __('Admin Configuration') }}
    </div>
    <!-- Nav Item - Admin Setup LMS -->
    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#ACLCollapse" aria-expanded="true" aria-controls="ACLCollapse">
            <i class="fas fa-fw fa-users-cog"></i>
            <span>Akun Manajemen</span>
        </a>
        <div id="ACLCollapse" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item" href="{{ route('user.index') }}">{{ __('User') }}</a>
                <a class="collapse-item" href="{{ route('role.index') }}">{{ __('Role') }}</a>
                <a class="collapse-item" href="{{ route('permission.index') }}">{{ __('Permission') }}</a>
            </div>
        </div>

    </li>

    <li class="nav-item">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#AnalyticsCollapse" aria-expanded="true" aria-controls="AnalyticsCollapse">
            <i class="fas fa-fw fa-chart-bar"></i>
            <span>{{ __('Analytics') }}</span>
        </a>
        <div id="AnalyticsCollapse" class="collapse" aria-labelledby="headingAnalytics" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="collapse-item" href="{{ route('panel.analytics.dda-metrics') }}">{{ __('Evaluasi Metrik DDA') }}</a>
                <a class="collapse-item" href="{{ route('panel.analytics.dda-comparison') }}">{{ __('Analisis DDA vs Non-DDA') }}</a>
                <a class="collapse-item" href="{{ route('panel.analytics.spk-recommendations') }}">{{ __('Rekap SPK') }}</a>
            </div>
        </div>
    </li>

    @endrole

    <!-- Divider -->
    <hr class="sidebar-divider d-none d-md-block">
    <!-- Sidebar Toggler (Sidebar) -->
    <div class="text-center d-none d-md-inline">
        <button class="rounded-circle border-0" id="sidebarToggle"></button>
    </div>

</ul>
<!-- End of Sidebar -->

