...
class TopicLevelComparisonController extends Controller
{
    ...
    /**
     * Get available quizzes for a term
     */
    ...
    /**
     * Compare quiz-specific vs overall topic levels for a student
     */
    public function getQuizComparison(Request $request)
    {
        ...
        try {
            ...
            foreach ($participants as $participant) {
                // Get ONLY quiz-specific topic levels
                $quizLevels = $this->aggregateTopicLevelsBySessionable(
                    $participant,
                    $sessionableId,
                    null // both DDA and Non-DDA
                );

                // Get overall topic levels (ALL quizzes)
                $overallLevels = $this->aggregateTopicLevelsByMode(
                    $participant,
                    null, // no session filter
                    null, // no sessionable filter - get ALL
                    null  // both DDA and Non-DDA
                );

                $allTopics = $allTopics
                    ->merge(array_keys($quizLevels))
                    ->merge(array_keys($overallLevels));
                ...
                $quizData[] = array_merge($studentInfo, [
                    'levels' => $quizLevels,
                ]);
    ...
    /**
     * Aggregate topic levels for a specific sessionable/quiz
     *
     * @param Participant $participant
     * @param int $sessionableId
     * @param bool|null $useDda - true=DDA only, false=Non-DDA only, null=all
     * @return array Topic levels keyed by topic name
     */
    private function aggregateTopicLevelsBySessionable(Participant $participant, $sessionableId, $useDda = null)
    {
        $topicLevels = [];

        // Get workouts specifically for this sessionable
        $workouts = $participant->Workout()
        ...
        foreach ($workouts as $workout) {
            ...
            // Process RestartLogs topic_levels
            foreach ($logs as $log) {
                if ($log->topic_levels) {
                    $logTopics = is_string($log->topic_levels)
                        ? json_decode($log->topic_levels, true)
            ...
            // Also aggregate from WorkoutQuizLogs
            $quizLogs = $workout->WorkOutQuiz()
                ->with('Question')
                ->get();
